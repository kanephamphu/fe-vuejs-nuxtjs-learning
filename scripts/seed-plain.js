...
console.log('Seeding database using plain JS...');
...
for (const [moduleName, lessonsList] of Object.entries(modules)) {
  for (const lessonData of lessonsList) {
    ...
    const existing = sqlite.prepare('SELECT id FROM lessons WHERE slug = ?').get(slug);

    if (existing) {
      sqlite.prepare('UPDATE lessons SET title = ?, topic = ?, content = ?, practice_content = ?, complexity = ? WHERE id = ?')
        .run(lessonData.title, lessonData.topic, content, practiceContent, lessonData.level, existing.id);
      
      sqlite.prepare('UPDATE exercises SET instructions = ? WHERE lesson_id = ?')
        .run(exerciseInstructions, existing.id);
      console.log(`Updated: ${slug}`);
    } else {
      const result = sqlite.prepare('INSERT INTO lessons (module, topic, title, slug, content, practice_content, complexity) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(moduleName, lessonData.topic, lessonData.title, slug, content, practiceContent, lessonData.level);
      
      sqlite.prepare('INSERT INTO exercises (lesson_id, instructions, starter_code, solution_code) VALUES (?, ?, ?, ?)')
        .run(result.lastInsertRowid, exerciseInstructions, `// Starter code for ${lessonData.title}\n`, `// Solution`);
